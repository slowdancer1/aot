    ...
    def evaluating(self):
        ...
        with torch.no_grad():
            for seq_idx, seq_dataset in enumerate(self.dataset):
                ...
                #engine1专门用于计算gc的梯度
                engine1=build_engine(cfg.MODEL_ENGINE,
                                             phase='train',
                ...
                for frame_idx, samples in enumerate(seq_dataloader):
                    ...
                    for aug_idx in range(len(samples)):
                        ...
                        if frame_idx == 0:
                            ...
                            engine.add_reference_frame(current_img,
                                                       ...
                                                       frame_step=0,
                                                       obj_nums=obj_nums)
                            first_img = current_img
                            first_mask = _current_label
                        else:
                            if aug_idx == 0:
                                seq_timers.append([])
                    ...
                    if frame_idx > 0:
                        ...
                        #print(torch.cat(pred_label[0][0],dim=0))
                        #print()
                            ...
                            if not cfg.MODEL_USE_PREV_PROB:
                                ...
                                for aug_idx in range(len(samples)):
                                    ...
                                    if frame_idx < 10:
                                        ...
                                        for i in range(10):
                                            tmp = current_label.detach()
                                            tmp.requires_grad = True
                                            #添加当前帧并计算梯度
                                            engine1.add_reference_frame(current_img,tmp,frame_idx,obj_nums,gc=True)
                                            loss = engine1.get_loss(first_img, first_mask)
                                            
                                            loss.backward(retain_graph=True)

                                            current_label = current_label - 10000 * tmp.grad
