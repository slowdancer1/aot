...
class AOTEngine(nn.Module):
    ...
    def get_loss(self, first_img, first_mask):
        first_img_embs, _ = self.encode_one_img_mask(first_img, first_mask)
        self.curr_enc_embs = first_img_embs
        self.curr_lstt_output = self.AOT.LSTT_forward(first_img_embs,
                                                      self.long_term_memories,
                                                      self.short_term_memories,
                                                      None,
                                                      pos_emb=self.pos_emb,
                                                      size_2d=self.enc_size_2d)
        loss, _, _ = self.generate_loss_mask(
                first_mask, 50000, return_prob=True)
        return loss
    ...
    def add_reference_frame(self,
        ...
        if self.pos_emb is None:
            ...
            self.pos_emb = self.AOT.get_pos_emb(curr_enc_embs[-1]).expand(
                self.batch_size, -1, -1,
                -1).view(self.batch_size, -1, self.enc_hw).permute(2,
                 0, 1)
        ...
        if gc or self.long_term_memories is None:
